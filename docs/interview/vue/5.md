...
```
效果：
![image](https://github.com/DMQ/mvvm/raw/master/img/1.gif)
...
4、实现一个订阅器Dep，维护一个数组，用来收集订阅者Watcher，数据变动触发notify，再调用订阅者的update方法

4、mvvm入口函数，整合以上三者。

上述流程如图所示：
![image](https://raw.githubusercontent.com/DMQ/mvvm/master/img/2.png)
### 1、实现Observer
...
```
这样我们已经可以监听每个数据的变化了，那么监听到变化之后就是怎么通知订阅者了，所以接下来我们需要实现一个消息订阅器（Dep），很简单，维护一个数组，用来收集订阅者，数据变动触发notify，再调用订阅者的update方法，代码改善之后是这样：
```javascript
// ... 省略
function defineReactive(data, key, val) {
	var dep = new Dep();
...
如图所示(图中的updater就是watcher)：
![image](https://github.com/DMQ/mvvm/raw/master/img/3.png)
...
![image](https://camo.githubusercontent.com/3845b9554e62650727fa7cae8f1c169060b879f7/68747470733a2f2f636e2e7675656a732e6f72672f696d616765732f646174612e706e67)
