...
```
（3）修改src/store/store.js，区分服务端的store和客户端的store。
```js
// 存储入口
...
```
## 9、将axios和redux结合
（1）首先在store.js里面，创建服务端和客户端的实例，然后传入`thunk.withExtraArgument`的参数里面。
```js
// 引入axios
